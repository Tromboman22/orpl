...
def test_crfilter_single_is_accurate(gen_synthetic_nylon):
    ...
    for i in range(cr_sig.shape[0]):
        ...
        assert (
            cr_sig[i][cr_position] > filtered_signal[cr_position]
        ), "crfilter_single did not accurately remove cosmic rays"
...
def test_crfilter_multi_detects_cosmic_rays(sample_data_bacon):
    ...
    # add cosmic rays
    for i in range(5):  # try 5 times to increase difficulty
        cr_signals = data[i]
        rand0 = np.random.randint(0, cr_signals.shape[0])  # for each spectrum
        rand1 = np.random.randint(0, cr_signals.shape[1])  # position to add cosmic ray
        cr_signals[rand0][rand1] += np.random.uniform(1.7e3, 3.5e3)
        rand2 = np.random.randint(0, cr_signals.shape[0])  # for each spectrum
        rand3 = np.random.randint(0, cr_signals.shape[1])  # position to add cosmic ray
        cr_signals[rand2][rand3] += np.random.uniform(1.7e3, 3.5e3)
        filtered_signals = crfilter_multi(cr_signals, 3, 0.04)
        assert (
            filtered_signals[rand0][rand1] < cr_signals[rand0][rand1]
        ), "crfilter_multi did not remove all cosmic rays 5 times in a row"
        assert (
            filtered_signals[rand2][rand3] < cr_signals[rand2][rand3]
        ), "crfilter_multi did not remove all cosmic rays 5 times in a row"
