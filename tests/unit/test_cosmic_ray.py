...
def cosmic_rays_bacon(sample_data_bacon):
    ...
    for s in signals:
        ...
        s[rand_sig][rand_pos] += 4e+3  # add cosmic ray spike
    return signals
...
def test_crfilter_single_invalid_input(synthetic_nylon_with_cosmic_rays):
    ...
    with pytest.raises(AttributeError) as e: # Input is regular array rather than np.ndarray
    ...
    with pytest.raises(ValueError) as e: # Invalid width input
        crfilter_single(synthetic_nylon_with_cosmic_rays, -1), (f"Width as negative number: {e}")
    ...
    with pytest.raises(ValueError) as e: # Invalid std_factor input
        crfilter_single(synthetic_nylon_with_cosmic_rays, 3, -0.1), (f"std_factor as negative number: {e}")
    with pytest.raises(TypeError) as e:
        crfilter_single(synthetic_nylon_with_cosmic_rays, 3, "invalid"), (f"std_factor as string: {e}")
...
def test_crfilter_single_correct_shape(synthetic_nylon_with_cosmic_rays):
    ...
    for i in range(signal.shape[0]):
        filtered_signal = crfilter_single(signal[i])
        assert filtered_signal.shape == signal[i].shape, "Output shape does not match input shape"
...
def test_crfilter_single_output_different_from_input(synthetic_nylon_with_cosmic_rays):
    ...
    for i in range(signal.shape[0]):
        filtered_signal = crfilter_single(signal[i])
        assert not np.array_equal(filtered_signal, signal[i]), "Filtered signal is identical to input signal"
...
def test_crfilter_single_handles_constant_array():
    ...
    assert np.array_equal(filtered_signal, constant_sig), "Filtered constant array should be identical to input"
    constant_sig[np.random.randint(0, 1000)] += np.random.uniform(0.15, 0.3) # add cosmic ray spike
...
def test_crfilter_single_cosmic_ray_edges(gen_synthetic_nylon):
    ...
    for i in range(signals.shape[0]):
        ...
        assert filtered[0] <= signals[i][0] and filtered[-1] <= signals[i][-1], "Edge cosmic ray not removed properly"
...
def test_crfilter_multi_no_cosmic_rays():
    ...
    assert np.array_equal(copy, signal), "Constant array edge case should get caught by cr_filter_multi"


@pytest.mark.cosmic_ray
def test_crfilter_multi_entire_signal_flagged_cosmic_ray(sample_data_bacon):
    signal = sample_data_bacon[0]
    copy = crfilter_multi(signal, width=3, disparity_threshold=-1.0)  # very low threshold to flag entire signal
    assert np.array_equal(copy, signal), "Entire signal as cosmic ray edge case should return unchanged array"
...
def test_crfilter_multi_correct_shape(cosmic_rays_bacon):
    signal = cosmic_rays_bacon[0]
    filtered_signal = crfilter_multi(signal)
    assert filtered_signal.shape == signal.shape, "Output shape does not match input shape"
...
def test_crfilter_multi_output_different_from_input(cosmic_rays_bacon):
    ...
    for i in range(5):  # try 5 different samples
        ...
        assert not np.array_equal(filtered_signal, signals), "Filtered signal is identical to input signal"
