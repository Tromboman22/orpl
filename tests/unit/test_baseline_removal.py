...
def gen_synthetic_nylon():  # gen_slist from demo #4
    ...
    for n_level in [0, 1, 5, 10, 15, 30]:
        s,r,b,_ = gen_synthetic_spectrum(preset='nylon',
                                        rb_ratio=0.2,
...
def test_imodpoly_with_invalid_inputs(gen_synthetic_nylon):
    ...
    with pytest.raises(ValueError) as e:
        ram, bas = imodpoly(signal[0], poly_order=-1), f"Imodpoly should break on negative poly_order: {e}"
    with pytest.raises(UnboundLocalError) as e:
        ram, bas = imodpoly(signal[0], poly_order=3, max_iter=0), f"Imodpoly should break on negative or zero as max_iter: {e}"
...
def test_imodpoly_baseline_below_signal(gen_synthetic_nylon):
    signal, raman, baseline = gen_synthetic_nylon
    ram, bas = imodpoly(signal[0])
    assert bas.mean() < signal[0].mean(), "Imodpoly returned baseline that exceeds signal values"
    assert ram.mean() < signal[0].mean(), "Imodpoly returned raman that exceeds signal values"
...
def test_imodpoly_over_constant_signal():
    ...
    ram, bas = imodpoly(signal[0])
    assert np.allclose(bas, 5.0), "Imodpoly failed to return constant baseline for constant signal"
    assert np.allclose(ram, 0.0), "Imodpoly failed to return zero raman for constant signal"
...
def test_morph_br_with_invalid_inputs(gen_synthetic_nylon):
    ...
    with pytest.raises(ValueError) as e:
        ram, bas = morph_br(signal[0], hws=-1), f"morph_br should break on negative hws: {e}"
...
def test_morph_br_baseline_below_signal(gen_synthetic_nylon):
    signal, raman, baseline = gen_synthetic_nylon
    ram, bas = morph_br(signal[0], hws=3)
    assert bas.mean() < signal[0].mean(), "morph_br returned baseline that exceeds signal values"
    assert ram.mean() < signal[0].mean(), "morph_br returned raman that exceeds signal values"
...
def test_morph_br_over_constant_signal():
    signal = np.full((5, 1000), 5.0)
    ram, bas = morph_br(signal[0], hws=3)
    assert np.allclose(bas, 5.0), "morph_br failed to return constant baseline for constant signal"
    assert np.allclose(ram, 0.0), "morph_br failed to return zero raman for constant signal"
...
def test_bubblefill_with_invalid_inputs(gen_synthetic_nylon):
    ...
    with pytest.raises(np._core._exceptions._UFuncNoLoopError) as e:
        ram, bas = bubblefill("invalid"), f"bubblefill should break on string input: {e}"
    with pytest.raises(ValueError) as e:
        fake_arr = np.full((5,1000), 1.0)
    ...
    assert np.allclose(ram, 0, atol=0.01), "bubblefill failed to return raman equal to zero for negative min_bubble_width"
    with pytest.raises(TypeError) as e:
        ram, bas = bubblefill(signal[0], min_bubble_widths=0.5), f"bubblefill should break on float min_bubble_width: {e}"
...
def test_bubblefill_output_shape(gen_synthetic_nylon):
    ...
    assert bas.shape == signal[0].shape, "bubblefill returned baseline of incorrect shape"
...
def test_bubblefill_baseline_below_signal(gen_synthetic_nylon):
    signal, raman, baseline = gen_synthetic_nylon
    ram, bas = bubblefill(signal[0], min_bubble_widths=3)
    assert bas.mean() < signal[0].mean(), "bubblefill returned baseline that exceeds signal values"
    assert ram.mean() < signal[0].mean(), "bubblefill returned raman that exceeds signal values"
...
def test_bubblefill_over_constant_signal():
    signal = np.full((5, 1000), 5.0)
    ram, bas = bubblefill(signal[0], min_bubble_widths=3)
    assert np.allclose(bas, 5.0), "bubblefill failed to return constant baseline for constant signal"
    assert np.allclose(ram, 0.0), "bubblefill failed to return zero raman for constant signal"
